infectedplot <- function(beta,gamma,S0,I0,R0,tau0,maxtime,nsims){
  N <- S0 + I0 + R0
  ...
  #plot the number infected for all simulations --> WORKS
  plot(1,type="n",xlab="Time",ylab="Number of Infected",
       main=NULL,xlim=c(0, maxt), 
       ylim=c(0, max(ninf,na.rm=TRUE)))
  
  for (i in 1:nsims) {
    lines(sims[[i]][,4], sims[[i]][,2], col=i)
  }
}